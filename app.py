...
def predict(image_file, model, transform, label_encoder, device):
    ...
    class_indices = np.argsort(all_probs)[::-1][:5]
    top_classes = [label_encoder.inverse_transform([i])[0] for i in class_indices]
    top_probs = [all_probs[i] * 100 for i in class_indices]

    os.remove(temp_path)

    return class_name, confidence, top_classes, top_probs
...
def main():
    ...
    st.markdown("<h1 style='text-align:center;color:#2E7D32;'>🌿 Plant Disease Classifier</h1>", unsafe_allow_html=True)
    ...
    with col1:
        ...
        with st.expander("Available Classes"):
            st.table(pd.DataFrame({"Diseases": [format_class(c) for c in class_names]}))
        ...
        if uploaded_file:
            if isinstance(uploaded_file, str):
                ...
                with open(uploaded_file, "rb") as f:
                    file_obj = type("obj", (object,), {"getvalue": lambda: f.read()})
            else:
                image = Image.open(uploaded_file)
            ...
            if model_loaded:
                class_name, confidence, top_classes, top_probs = predict(
                    file_obj, model, transform, label_encoder, device
                )

                st.success(f"**Diagnosis:** {format_class(class_name)}")
                st.info(f"**Confidence:** {confidence:.2f}%")

                df = pd.DataFrame({
                    "Disease": [format_class(c) for c in top_classes],
    ...
    with st.sidebar:
        st.header("About")
        st.write("CNN-based plant disease classification app.")
        st.write("Dataset: PlantVillage")
        st.write(f"Classes: {len(class_names)}" if model_loaded else "Model not loaded")
